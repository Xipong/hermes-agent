...
const kanban = (enabled = false): ToolsetInfo => ({
  configured: true, description: 'Board tools', enabled, label: 'Kanban', name: 'kanban', tools: ['kanban_create']
})
...
it('keeps Desktop separate and binds an in-flight tool grant to its connection/profile', async () => {
  ...
  await act(async () => { finish({ ok: true, name: 'kanban', enabled: true }); await save })
  await waitFor(() => expect(notify).toHaveBeenCalledWith(expect.objectContaining({
    kind: 'success', message: expect.stringContaining('Open a new chat')
  })))
  expect(agentSwitch().getAttribute('aria-checked')).toBe('false')
...
it.each(['missing', 'read-error', 'write-error', 'invalid-receipt'] as const)(
  'does not pretend the tools were enabled on %s and leaves the board usable', async mode => {
    if (mode === 'missing') vi.mocked(getToolsets).mockResolvedValue([])
    if (mode === 'read-error') vi.mocked(getToolsets).mockRejectedValueOnce(new Error('offline'))
    if (mode === 'write-error') vi.mocked(setToolsetEnabled).mockRejectedValue(new Error('write denied'))
    if (mode === 'invalid-receipt') vi.mocked(setToolsetEnabled).mockResolvedValue({ ok: false, name: 'kanban', enabled: true })
