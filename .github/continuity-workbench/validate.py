...
new_py = git('diff', '--name-only', '--diff-filter=A', BASE, 'HEAD').splitlines()
new_py = [p for p in new_py if p.endswith('.py')]
...
if git('status', '--porcelain').strip():
    ...
    subprocess.run(['git', 'commit', '-m', 'style: normalize integrated sources with pinned project tools'], check=True)
...
run('python-targeted', ['bash', 'scripts/run_tests.sh', '-j', '2', '--file-retries', '0', '--file-timeout', '180',
    ...
    'tests/gateway/test_api_server_compaction_projection.py', 'tests/test_cli_quiet_stdout_leak.py',
...
run('desktop-targeted', ['npm', 'run', 'test:ui', '--', '--maxWorkers=2',
    'src/lib/chat-messages', 'src/app/session/hooks/use-message-stream'], cwd=ROOT / 'apps/desktop', timeout=720)
run('lint-typescript', [str(ROOT / 'node_modules/.bin/eslint'), '--max-warnings', '0', *[str(ROOT / p) for p in ts]], timeout=240)
# Mutation controls: revert only the actual source mechanisms in this checkout,
# run fresh isolated subprocesses, and restore before committing/publishing anything.
...
# Expected red controls are successful evidence only when they actually fail.
