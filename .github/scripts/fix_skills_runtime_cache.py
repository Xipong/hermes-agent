...
p = Path('tools/skills_sync_optional.py')
s = p.read_text()
start = s.index('def _skill_file_list(')
end = s.index('\n\ndef _load_hub_lock(', start)
s = s[:start] + '''# Only generated runtime state, never generic cache/ or arbitrary dotfiles. This
...
''' + s[end:]
p.write_text(s)
p = Path('tools/skills_sync.py')
s = p.read_text().replace('from tools.skills_sync_bundled_ops import _is_tracked_user_modification\n', '')
s = s.replace('from tools.skills_sync_optional import _backfill_optional_provenance, _read_hub_install_paths', '''from tools.skills_sync_optional import (
    _backfill_optional_provenance, _ignore_runtime_cache, _is_runtime_cache, _read_hub_install_paths,
)''')
s = s.replace('def _dir_hash(directory: Path) -> str:\n    """MD5 over relative paths + contents of every file in a directory."""', '''def _dir_hash(directory: Path, *, include_runtime_cache: bool = False) -> str:
...
''' + s[idx:]
s = s.replace('shutil.copytree(src, dest)', 'shutil.copytree(src, dest, ignore=_ignore_runtime_cache)')
s = s.replace('shutil.copytree(skill_src, dest)', 'shutil.copytree(skill_src, dest, ignore=_ignore_runtime_cache)')
s = s.replace('if _dir_hash(candidate) != origin_hash:', 'if not _matches_origin_hash(candidate, origin_hash):')
s = s.replace('if _is_tracked_user_modification(origin_hash, user_hash):', 'if not _matches_origin_hash(dest, origin_hash, user_hash):')
p.write_text(s)
p = Path('tools/skills_sync_bundled_ops.py')
s = p.read_text().replace('if origin_hash and dest.exists() and _is_tracked_user_modification(origin_hash, ss._dir_hash(dest)):', 'if origin_hash and dest.exists() and not ss._matches_origin_hash(dest, origin_hash):')
s = s.replace('if ss._dir_hash(dest) != origin_hash:', 'if not ss._matches_origin_hash(dest, origin_hash):')
i = s.index('def _is_tracked_user_modification(')
j = s.index('def _bundled_state(', i)
p.write_text(s[:i] + s[j:])

Path('tests/tools/test_skills_runtime_cache.py').write_text('''"""Runtime caches must not take ownership of updater-managed skill packages."""
