...
const items = [{ type: 'reasoning', id: 'rs', summary: [{ type: 'summary_text', text: 'Inspect' }, { type: 'summary_text', text: 'Check' }] }]
...
describe('native reasoning identity without a second commentary projector (#79727)', () => {
  it('uses only backend-authorized identities matching all displayed reasoning', () => {
    const row = { role: 'assistant' as const, content: 'Done.', reasoning: 'Inspect\nCheck\n\nPublic.',
      display_reasoning: 'Inspect\nCheck', display_commentary: ['Public.'], display_reasoning_items: items,
      codex_reasoning_items: [{ type: 'reasoning', id: 'private', summary: [{ type: 'summary_text', text: 'Not authorized' }] }] }
    ...
    expect(message.parts.filter(part => part.type === 'reasoning').map(part => [part.sourceId, part.text])).toEqual([
      ['rs:summary:0', 'Inspect'], ['rs:summary:1', 'Check']
    ])
  ...
  it('never joins native sources across text, tools, completion, or legacy chunks', () => {
    ...
    for (const boundary of [assistantTextPart('Public', 2), toolCallPart('tc')]) {
      expect(appendReasoningPart([first, boundary], 'After', 3, 'rs')).toEqual([
        first, boundary, reasoningPart('After', 3, 'rs')
      ])
    }
    ...
    expect(appendReasoningPart([first], ' continuation', 2, 'rs')).toEqual([reasoningPart('Before continuation', 1, 'rs')])
  })
})
