...
export function commentaryAppEnv(sandbox: Sandbox): Record<string, string> {
  ...
  const python = process.env.HERMES_DESKTOP_PYTHON ?? ['.venv', 'venv']
    .map(dir => path.join(REPO_ROOT, dir, ...suffix)).find(candidate => fs.existsSync(candidate))
...
export async function seedCodexCommentarySession(sandbox: Sandbox, sessionId: string): Promise<void> {
  ...
  await run(env.HERMES_DESKTOP_PYTHON, [
    path.join(import.meta.dirname, 'seed-codex-commentary-session.py'),
    '--hermes-home', sandbox.hermesHome,
    '--state-db', path.join(sandbox.hermesHome, 'state.db'),
