"""Validation-only extension for diagnostic CLI consumers of PR #115033."""
from pathlib import Path
import sys

CLI_TEST = '''"""CLI diagnostics must report the same target error without masking it as a missing install (#115033)."""
...
def prepare_tests():
    path = Path("tests/hermes_cli/test_computer_use_target_status.py")
    assert not path.exists()
    path.write_text(CLI_TEST, encoding="utf-8", newline="\n")


def prepare_source():
    path = Path("hermes_cli/subcommands/computer_use.py")
    ...
    path = Path("tools/computer_use/doctor.py")
    text = path.read_text(encoding="utf-8")
    text = replace_once(text,
