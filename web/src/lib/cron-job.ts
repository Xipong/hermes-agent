...
export function buildCronJobPayload(form: CronJobFormState): CronJobMutation {
  ...
  const contextFrom = splitCronList(form.context_from).filter(item => item.toLowerCase() !== 'self')
...
export function cronJobHasExecutionContent(job: Pick<CronJobMutation, 'prompt' | 'skills' | 'script'>): boolean {
...
export function cronJobFormFromJob(job: CronJob): CronJobFormState {
  ...
  const continuity =
    Boolean((job as { continuity?: boolean }).continuity) || storedRefs.some(item => item.toLowerCase() === 'self')
  const externalRefs = storedRefs.filter(item => item.toLowerCase() !== 'self')
  return {
    name: asString(job.name),
    prompt: asString(job.prompt),
    schedule: asString(job.schedule?.expr) || asString(job.schedule?.run_at) || asString(job.schedule_display),
