...
export default function SystemPage() {
  ...
  const runDebugShare = useCallback(async () => {
    ...
    try {
      ...
      showToast(`Uploaded ${n} paste${n === 1 ? '' : 's'}${res.redacted ? ' (redacted)' : ''}`, 'success')
    } catch (e) {
  ...
  const applyUpdate = async () => {
    ...
    if (status?.can_update_hermes === false) {
      showToast('Hermes updates are managed outside this dashboard.', 'success')
      return
    }
    try {
      const resp = await api.updateHermes()
      if (!resp.ok) {
        showToast(resp.message ?? "Updates don't apply from this dashboard.", 'success')
        return
      }
    ...
    <div className="flex flex-col gap-8">
        ...
        >
          ...
          >
            ...
            <header className="p-5 pb-3 border-b border-border">
              <h2 className="font-mondwest text-display text-base tracking-wider">New shell hook</h2>
            </header>
            <div className="p-5 grid gap-4">
              ...
              <p className="text-xs text-warning">
                Shell hooks run arbitrary commands on this host. Only add scripts you trust. Takes effect on the next
                gateway/session restart.
              </p>
        ...
        <Card>
          <CardContent className="flex flex-col gap-3 py-4">
            <div className="flex items-center gap-3">
              ...
              {portal?.provider && (
                <span className="text-sm text-muted-foreground">inference provider: {portal.provider}</span>
              )}
              <a
            ...
            {portal?.features && portal.features.length > 0 && (
              <div className="flex flex-col gap-1 border-t border-border pt-3">
                <span className="text-xs uppercase tracking-wider text-muted-foreground">Tool Gateway routing</span>
      ...
      <section className="flex flex-col gap-3">
        ...
        <Card>
          <CardContent className="flex items-center justify-between py-4">
            <div className="flex items-center gap-3">
              <Badge tone={gatewayRunning ? 'success' : 'secondary'}>{gatewayRunning ? 'running' : 'stopped'}</Badge>
              <span className="text-sm text-muted-foreground">
        ...
        <Card>
          <CardContent className="flex flex-col gap-4 py-4">
            <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted-foreground">
              <span>
                External provider:{' '}
                <span className="font-mono text-foreground">{memory?.active || 'built-in only'}</span>
              </span>
            ...
            {activeMemoryProvider?.status === 'missing' && (
              <p className="border border-destructive/50 px-3 py-2 text-xs text-destructive">
                The configured provider is no longer installed. Switch to built-in memory or configure another provider
                in Plugins.
              </p>
            )}
            ...
            <div className="flex flex-wrap items-center gap-3 border-t border-border pt-3">
              <span className="text-xs text-muted-foreground">
                Built-in files — MEMORY.md: {formatBytes(memory?.builtin_files.memory ?? 0)} · USER.md:{' '}
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-xs text-muted-foreground max-w-prose">
                    Uploads system info + logs to a public paste service and returns links to send the Hermes team.
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="flex items-center gap-1 text-xs text-muted-foreground">
                      ...
                      auto-deletes in {Math.round(shareResult.auto_delete_seconds / 3600)}h
                    </span>
                  </div>
        ...
        {hooks?.hooks.map((h: HookEntry, i) => (
          <Card key={`${h.event}-${i}`}>
            <CardContent className="flex items-center gap-3 py-3">
              ...
              {h.matcher && <span className="text-xs text-muted-foreground">matcher: {h.matcher}</span>}
              <span className="font-mono text-xs truncate flex-1">{h.command}</span>
              {h.executable === false && <Badge tone="destructive">not executable</Badge>}
              <Badge tone={h.allowed ? 'success' : 'warning'}>{h.allowed ? 'allowed' : 'not approved'}</Badge>
              <Button
                ghost
                size="icon"
                className="text-destructive"
