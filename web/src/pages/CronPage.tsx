  ...
  const handlePauseResume = async (job: CronJob) => {
    try {
      ...
      if (isPaused) {
        ...
        showToast(`${t.cron.resume}: "${truncateText(getJobTitle(job), 30)}"`, 'success')
      } else {
        await api.pauseCronJob(job.id, profile)
        showToast(`${t.cron.pause}: "${truncateText(getJobTitle(job), 30)}"`, 'success')
      }
      loadJobs(selectedProfile)
    } catch (e) {
    ...
    onDelete: useCallback(
      async (key: string) => {
        ...
        try {
          ...
          showToast(`${t.common.delete}: "${job ? truncateText(getJobTitle(job), 30) : id}"`, 'success')
          loadJobs(selectedProfile)
        } catch (e) {
          showToast(`${t.status.error}: ${e}`, 'error')
          throw e
                ...
                <CardContent className="flex items-start gap-4 py-4">
                  <div className="flex-1 min-w-0">
                    ...
                    {job.last_delivery_error && (
                      <p className="text-xs text-destructive mt-1">delivery: {job.last_delivery_error}</p>
                    )}
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    <Button ghost size="icon" title="Edit job" aria-label="Edit job" onClick={() => openEditModal(job)}>
                      <Pencil />
                    </Button>
