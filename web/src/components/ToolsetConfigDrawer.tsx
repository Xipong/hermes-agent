...
import type { ToolsetConfig, ToolsetInfo, ToolsetProvider } from '@/lib/api-types'
...
export function ToolsetConfigDrawer({ toolset, profile, onClose, onChanged }: Props) {
    ...
    const poll = async () => {
      try {
        ...
        if (st.running) {
          ...
          showToast(ok ? 'Post-setup complete' : 'Post-setup finished with errors', ok ? 'success' : 'error')
  ...
  const handleToggle = async (next: boolean) => {
    ...
    try {
      ...
      setEnabled(next)
      showToast(`${toolset.label || toolset.name} ${next ? 'enabled' : 'disabled'}`, 'success')
      onChanged()
    } catch {
  ...
  const handleSelectProvider = async (provider: ToolsetProvider) => {
    ...
    try {
      ...
      showToast(e instanceof Error ? e.message : 'Failed to select provider', 'error')
    } finally {
      setSelecting(null)
    }
  }
  ...
  const handleSaveKeys = async (provider: ToolsetProvider) => {
    ...
    try {
      ...
      showToast(
        res.saved.length ? `Saved ${res.saved.length} key${res.saved.length > 1 ? 's' : ''}` : 'Nothing to save',
        'success'
      )
      onChanged()
    } catch (e) {
      showToast(e instanceof Error ? e.message : 'Failed to save keys', 'error')
    } finally {
  ...
  const handleRunPostSetup = async (provider: ToolsetProvider) => {
    ...
    try {
      ...
      setPostSetupRunning(false)
      showToast(e instanceof Error ? e.message : 'Failed to start post-setup', 'error')
    }
  }
        ...
        <header className="p-5 pb-3 border-b border-border">
          ...
          <div className="mt-3 flex items-center gap-2">
            ...
            <span className="text-xs text-muted-foreground">
              {enabled ? `Enabled for ${platformText}` : `Disabled for ${platformText}`}
            </span>
          </div>
        </header>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto p-5 pt-4 space-y-4">
          {loading ? (
            ...
            <p className="text-sm text-muted-foreground py-6 text-center">
              This toolset has no configurable backends — toggle it on or off above. It works with no provider selection
              or API keys.
            </p>
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    ...
                    {isActive ? (
                      ...
                      >
                        {selecting === provider.name ? <Loader2 className="h-3 w-3 animate-spin" /> : 'Select'}
                      </Button>
                    )}
                  </div>
                  ...
                  {provider.env_vars.length > 0 && (
                    <div className="mt-3 space-y-2.5">
                      {provider.env_vars.map(ev => (
                        <div key={ev.key} className="space-y-1">
                          ...
                          <Input
                            ...
                            placeholder={isSet[ev.key] ? '•••••••• (saved — leave blank to keep)' : ev.prompt || ev.key}
                            value={drafts[ev.key] ?? ''}
                            onChange={e =>
                              setDrafts(prev => ({
                                ...prev,
                      ...
                      >
                        {savingProvider === provider.name ? <Loader2 className="h-3 w-3 animate-spin" /> : 'Save keys'}
                      </Button>
                    </div>
                  )}
                  ...
                  {provider.post_setup && (
                    <div className="mt-3 border-t border-border pt-3">
                      <p className="text-xs text-muted-foreground mb-1.5">
                        This backend needs a one-time install <span className="font-mono">({provider.post_setup})</span>
                      ...
                      >
                        {postSetupRunning && postSetupKey === provider.post_setup ? 'Installing…' : 'Run setup'}
                      </Button>
                    </div>
                  )}
                </div>
          ...
          {(postSetupRunning || postSetupLog.length > 0) && (
            <div className="border border-border">
              <div className="flex items-center gap-2 px-3 py-1.5 border-b border-border bg-muted/30">
                ...
                <span className="text-xs font-mono text-muted-foreground">post-setup: {postSetupKey}</span>
