...
import type { StatusResponse, MemoryPressureStatus, DiskPressureStatus } from '@/lib/api-types'
...
function statusWithDisk(disk: DiskPressureStatus | undefined, memory?: MemoryPressureStatus): StatusResponse {
...
describe('MemoryPressureBanner', () => {
  it('renders nothing for null status or healthy memory', async () => {
    ...
    await rerender(<MemoryPressureBanner status={statusWith({ pressure: 'ok' })} />)
    expect(banner()).toBeNull()
  ...
  it('renders nothing for unknown pressure (absence of evidence)', async () => {
    await render(<MemoryPressureBanner status={statusWith({ pressure: 'unknown' })} />)
    expect(banner()).toBeNull()
  })

  it('shows the elevated warning', async () => {
    await render(<MemoryPressureBanner status={statusWith({ pressure: 'elevated' })} />)
  ...
  it('shows the OOM-restart notice even when current pressure is ok', async () => {
    await render(<MemoryPressureBanner status={statusWith({ pressure: 'ok', last_boot_suspected_oom: true })} />)
    expect(banner()?.textContent).toContain('restarted unexpectedly, most likely because it ran out of memory')
  })
  ...
  it('dismissal hides the banner and persists across re-renders', async () => {
    await render(<MemoryPressureBanner status={statusWith({ pressure: 'elevated' })} />)
    const dismiss = container.querySelector('[data-testid="memory-pressure-banner"] button') as HTMLButtonElement
    await act(async () => dismiss.click())
    expect(banner()).toBeNull()
    await rerender(<MemoryPressureBanner status={statusWith({ pressure: 'elevated' })} />)
    expect(banner()).toBeNull()
  })
  ...
  it('escalation to critical re-opens a dismissed banner', async () => {
    ...
    await act(async () => dismiss.click())
    expect(banner()).toBeNull()
    await rerender(<MemoryPressureBanner status={statusWith({ pressure: 'critical' })} />)
    expect(banner()?.textContent).toContain('almost out of memory')
  })
  ...
  it('recovery to ok resets a dismissed live-pressure banner for the next episode', async () => {
    await render(<MemoryPressureBanner status={statusWith({ pressure: 'critical' })} />)
    const dismiss = container.querySelector('[data-testid="memory-pressure-banner"] button') as HTMLButtonElement
    await act(async () => dismiss.click())
    expect(banner()).toBeNull()
  ...
  it('unknown pressure (stale heartbeat) does NOT reset live dismissals', async () => {
    ...
    const dismiss = container.querySelector('[data-testid="memory-pressure-banner"] button') as HTMLButtonElement
    await act(async () => dismiss.click())
    await rerender(<MemoryPressureBanner status={statusWith({ pressure: 'unknown' })} />)
    expect(banner()).toBeNull()
    await rerender(<MemoryPressureBanner status={statusWith({ pressure: 'elevated' })} />)
    expect(banner()).toBeNull()
  })
  ...
  it('renders nothing for healthy or unknown disk', async () => {
    await render(<MemoryPressureBanner status={statusWithDisk({ pressure: 'ok', free_mb: 5000 })} />)
    expect(banner()).toBeNull()
    await rerender(<MemoryPressureBanner status={statusWithDisk({ pressure: 'unknown' })} />)
    expect(banner()).toBeNull()
  })

  it('shows the disk-critical warning with free-space detail', async () => {
    await render(<MemoryPressureBanner status={statusWithDisk({ pressure: 'critical', free_mb: 120 })} />)
  ...
  it('shows the disk-elevated warning', async () => {
    await render(<MemoryPressureBanner status={statusWithDisk({ pressure: 'elevated', free_mb: 900 })} />)
  ...
  it('disk critical outranks memory critical', async () => {
    ...
    await render(
      <MemoryPressureBanner status={statusWithDisk({ pressure: 'critical', free_mb: 100 }, { pressure: 'critical' })} />
    )
    expect(banner()?.textContent).toContain('disk is almost full')
  })
  ...
  it('dismissing a disk warning does not mask a later memory warning', async () => {
    ...
    expect(banner()).toBeNull()
    await rerender(
      <MemoryPressureBanner status={statusWithDisk({ pressure: 'elevated', free_mb: 900 }, { pressure: 'elevated' })} />
    )
    expect(banner()?.textContent).toContain('running low on memory')
  })
  ...
  it('disk escalation to critical re-opens a dismissed disk banner', async () => {
    ...
    await act(async () => dismiss.click())
    expect(banner()).toBeNull()
    await rerender(<MemoryPressureBanner status={statusWithDisk({ pressure: 'critical', free_mb: 150 })} />)
    expect(banner()?.textContent).toContain('disk is almost full')
  })

  it('disk recovery to ok resets disk dismissals for the next episode', async () => {
    await render(<MemoryPressureBanner status={statusWithDisk({ pressure: 'critical', free_mb: 150 })} />)
    const dismiss = container.querySelector('[data-testid="memory-pressure-banner"] button') as HTMLButtonElement
    await act(async () => dismiss.click())
    // User frees space (or grows the disk)...
    await rerender(<MemoryPressureBanner status={statusWithDisk({ pressure: 'ok', free_mb: 8000 })} />)
    expect(banner()).toBeNull()
  ...
  it('disk recovery does NOT reset memory dismissals (and vice versa)', async () => {
    ...
    await render(
      <MemoryPressureBanner status={statusWithDisk({ pressure: 'elevated', free_mb: 900 }, { pressure: 'critical' })} />
    )
    const dismiss = container.querySelector('[data-testid="memory-pressure-banner"] button') as HTMLButtonElement
    ...
    expect(banner()?.textContent).toContain('disk is filling up')
    const dismissDisk = container.querySelector('[data-testid="memory-pressure-banner"] button') as HTMLButtonElement
    await act(async () => dismissDisk.click())
    expect(banner()).toBeNull()
    // Disk recovers; memory still critical — its dismissal must survive.
    await rerender(
      <MemoryPressureBanner status={statusWithDisk({ pressure: 'ok', free_mb: 8000 }, { pressure: 'critical' })} />
    )
    expect(banner()).toBeNull()
  })
